...
namespace Systems.Details.Components
{
	[RequireComponent(typeof(RectTransform))]
	public class RangeSlider : Selectable, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		private const float HANDLE_TOUCH_TOL = 0.2f;
		...
		[SerializeField] private Image minHandleImage;
		[SerializeField] private RectTransform maxHandle;
		[SerializeField] private Image maxHandleImage;
		[SerializeField] private RectTransform middleHandle;
		[SerializeField] private Image fillImage;
		[SerializeField] private Color selectedColour;

		private Color _defaultMinHandleColour;
		private Color _defaultMaxHandleColour;
		private Color _defaultFillColour;

		[Header("Handles")] 
		[SerializeField] private float minHandleOffset;
		[SerializeField] private float maxHandleOffset;
		...
		protected override void Start()
		{
			...
			_defaultMinHandleColour = minHandleImage.color;
			_defaultMaxHandleColour = maxHandleImage.color;
			_defaultFillColour = fillImage.color;
		}
		...
		private void UpdateMiddleGraphic()
		{
			...
			middleHandle.anchorMin = Vector2.zero;
			middleHandle.anchorMax = Vector2.one;
			middleHandle.offsetMin = new Vector2(minHandle.anchoredPosition.x, 0);
			middleHandle.offsetMax = new Vector2(maxHandle.anchoredPosition.x, 0);
		}
		...
		public void OnBeginDrag(PointerEventData eventData)
		{
			...
			else
			{
				...
				else if (dragStartValue > dragStartMaxValue01)
				{
					...
					maxHandleImage.color = selectedColour;
				}
				else
				{
					handleType = HandleType.Both;
					fillImage.color = selectedColour;
				}
			}
		}
		...
		public void OnEndDrag(PointerEventData eventData)
		{
			...
			minHandleImage.color = _defaultMinHandleColour;
			maxHandleImage.color = _defaultMaxHandleColour;
			fillImage.color = _defaultFillColour;
		}
		...
		{
			handle.anchoredPosition = new Vector2(value01 * sliderBounds.rect.width - sliderBounds.rect.width + sliderBounds.offsetMax.x + maxHandleOffset, handle.anchoredPosition.y);
		}
		...
		{
			handle.anchoredPosition = new Vector2(value01 * sliderBounds.rect.width + sliderBounds.offsetMin.x + minHandleOffset, handle.anchoredPosition.y);
		}
		...
		{
			return 1 + ((handle.anchoredPosition.x - maxHandleOffset) - sliderBounds.offsetMax.x) / sliderBounds.rect.width;
		}
		...
		{
			return ((handle.anchoredPosition.x - minHandleOffset) - sliderBounds.offsetMin.x) / sliderBounds.rect.width;
		}
