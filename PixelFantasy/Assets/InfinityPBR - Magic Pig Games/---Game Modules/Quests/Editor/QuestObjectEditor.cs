...
namespace InfinityPBR.Modules
{
    ...
    public class QuestObjectEditor : Editor
    {
        ...
        private QuestsDrawer _modulesDrawer;
        
        private void OnEnable()
        {
            _modulesObject = (Quest)target;
            _modulesDrawer = CreateInstance<QuestsDrawer>();
        ...
        private string EditorPrefsString => "QuestObjectsEditor";
        private int MainPanelSelected => EditorPrefs.GetInt(EditorPrefsString, 0);
        
        private float _width;
        private int _columnWidth = 190;
        private Vector2 _scrollPosition;
        
        private QuestConditionDrawer questConditionDrawer;
        private QuestRewardDrawer questRewardDrawer;
        ...
        private List<LootItems> activeLootItems = new List<LootItems>();
        ...
        protected override void Setup()
        {
            ...
            {
                var drawerInstance = ScriptableObject.CreateInstance(type) as QuestConditionDrawer;
            ...
            {
                var drawerInstance = ScriptableObject.CreateInstance(type) as QuestRewardDrawer;
        ...
        protected override void Draw()
        {
            ...
            LabelGrey("Quest - " + Script.objectType);
        ...
        protected override void DrawLinkToDocs() => LinkToDocs("https://infinitypbr.gitbook.io/infinity-pbr/game-modules/quests");

        private void ChecksAndUpdates()
        {
            /*
            foreach (var step in _this.questSteps)
            {
                step.SetQuest(_this);
            }
            */
        /*       }
               ...
               {
                   var barOptions = new[] {"Main Settings", "Stat Effects", "Dictionaries"};
                   ToolbarMenuMain(barOptions, EditorPrefsString);
               }
               ...
               //private string SearchStringQuestReward => GetString("Quest Reward Search");
               ...
               private void UpdateList()
               {
                   ...
                   {
                       resultsQuestConditions = GameModuleObjects<QuestCondition>(false)
...
{
  foreach (var quest in GameModuleObjects<Quest>())
  {
      var tempName = quest.objectName;
      var tempType = quest.objectType;
      CheckName(_modulesObject);
      quest.CheckObjectType();
      var newCount = GameModuleObjects<Quest>().Count(x => x.objectName == quest.objectName);
      if (newCount > 1)
          Debug.LogError("There are " + newCount + " Quests named " + quest.objectName);
      if (tempName != quest.objectName || tempType != quest.objectType)
      {
          UpdateProperties();
