...
{
    [CreateAssetMenu(fileName = "Game Stat List Contains...", menuName = "Game Modules/Quest Condition/Game Stat List Contains", order = 1)]
    public class GameStatListContains : QuestCondition
    {
        [Header("Game Stat List Options")]
        ...
        public List<Stat> value;
        public List<string> types;
        
        public override bool ConditionMet<T>(QuestStep questStep, T obj)
        {
            // If the obj passed in is a GameStatList, we can skip to the check on whether
        ...
        public override bool ConditionMet(BlackboardNote blackboardNote) => ConditionMet(blackboardNote.valueGameStatList);
        
        private bool ConditionMet(GameStatList list)
        {
        ...
        private bool ConditionMetNoItems(GameStatList list)
        {
        ...
        private bool ConditionMetBasedOnTypes(GameStatList list)
        {
            if (types.Count == 0)
        ...
        private bool ConditionMetBasedOnItems(GameStatList list)
        {
            if (value.Count == 0)
                return ConditionMetNoItems(list);
            ...
            if (groupOfObjects == GroupOfObjects.Sum)
            {
                var sum = value.Sum(stat => list.Count(stat.Uid()));
            ...
            {
                if (valueContents == ValueContents.HasExactly)
                    return value.Any(stat => list.Count(stat.Uid()) == quantity);
            ...
            {
                if (valueContents == ValueContents.HasExactly)
                    return value.All(stat => list.Count(stat.Uid()) == quantity);
