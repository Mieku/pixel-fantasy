...
namespace InfinityPBR.Modules
{
    public class LootBoxesManager
    {
        ...
        // -------------------------------
        private LootBoxesDrawer _drawer = ScriptableObject.CreateInstance<LootBoxesDrawer>();
        ...
        private string FinalDestination => LocationToSave<LootBox>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        private readonly string _defaultSaveLocation = $"Game Module Objects/{ThisType}es/";
        ...
        private void ResetCache()
        {
            ...
            SetDictionaryDrawer();
            GameModuleObjectTypes<LootBox>(true);
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Loot Boxes")
                ManageObjects();
        ...
        {
            foreach(var moduleObject in SearchResults<LootBox>(SearchTypeIndex, SearchString))
            {
        ...
        private void ShowOff(LootBox moduleObject)
        {
            StartRow();
            ...
            // More here
            ShowLootRange(moduleObject);
            
            EndRow();
        }

        private void ShowOn(LootBox moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
        ...
        private void ShowLootRange(LootBox modulesObject)
        {
            if (modulesObject.MaxItems == 0)
            {
                Label("<i><b><color=#cc5555>No Items</color></b></i>", 150, false, false, true);
                return;
            }
            Label($"<color=#777777>{modulesObject.ForcedItems.StringWithColor("#999999")} - {modulesObject.MaxItems.StringWithColor("#999999")} Items</color>", 150, false, false, true);
        }

        private void DrawSearchBox()
        {
            var searchResults = SearchBoxWithType<LootBox>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        private void ManageCreateNew()
        {
            ...
            if (Button($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    ...
                    var newObjectType = CreateAssets<LootBox>(FinalDestination, NewObjectType, NewObjectTypeIndex, NewObjectsNameArray(NewObjectNames));
                    ...
                    GameModuleObjectsOfType<LootBox>(newObjectType, true);
                    ...
                    SearchTypeIndex = GameModuleObjectTypes<LootBox>(true).ToList().IndexOf(newObjectType);
                    GameModuleObjectsOfType<LootBox>(newObjectType, true);
