...
{
    public class LootItemsManager
    {
        // -------------------------------
        // UPDATE THESE FOR THIS TYPE
        // -------------------------------
        private LootItemsDrawer _drawer = ScriptableObject.CreateInstance<LootItemsDrawer>();
        ...
        private string[] GameModuleObjectTypes(bool recompute = false) => GameModuleObjectTypes<LootItems>(recompute);
        private LootItems[] GameModuleObjects(bool recompute = false) => GameModuleObjects<LootItems>(recompute);
        private LootItems[] GameModuleObjectsOfType(string type, bool recompute = false) => GameModuleObjectsOfType<LootItems>(type, recompute);
        private string FinalDestination => LocationToSave<LootItems>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        private void ResetCache()
        {
            ...
            SetDictionaryDrawer();
            GameModuleObjectTypes<LootItems>(true);
            GameModuleObjects<LootItems>(true);
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Loot Items")
                ManageObjects();
            
            SetAllDirty<LootItems>();
        }
        ...
        {
            foreach(var moduleObject in SearchResults<LootItems>(SearchTypeIndex, SearchString))
            {
        ...
        private void ShowOff(LootItems moduleObject)
        {
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
            ShowLinkButton(moduleObject);
            Label($"<b>{moduleObject.ObjectName}</b> <color=#777777><i>{moduleObject.ObjectType}</i></color>", 300, false, false, true);
            
            // More here
            ShowItemCount(moduleObject);
            ShowBoxCount(moduleObject);
            
            EndRow();
        }

        private void ShowOn(LootItems moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
        ...
        private void ShowItemCount(LootItems moduleObject)
        {
            Label($"<color=#999999><b>{moduleObject.itemObjects.Count}</b></color> <color=#777777>Items</color>", 100, false, false, true);
        }
        
        private void ShowBoxCount(LootItems moduleObject)
        {
            var boxCount = GameModuleObjects<LootBox>().Count(x => x.ContainsLootItems(moduleObject));
            if (boxCount == 0) return;
            
            Label($"<color=#999999><b>{boxCount}</b></color> <color=#777777>LootBoxes</color>", 100, false, false, true);
        }

        private void DrawSearchBox()
        {
            var searchResults = SearchBoxWithType<LootItems>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        {
            Label("<b><color=#99ffff>Loot Boxes</color></b> & <b><color=#99ffff>Loot Items</color></b> have <b><color=#99ffff>Dictionaries</color></b>, as do all <b><color=#99ffff>ModulesScriptableObject</color></b> objects, " +
        ...
        private void ManageCreateNew()
        {
            ...
            if (Button($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    ...
                    var newObjectType = CreateAssets<LootItems>(FinalDestination, NewObjectType, NewObjectTypeIndex, NewObjectsNameArray(NewObjectNames));
                    if (isExistingType)
                    {
