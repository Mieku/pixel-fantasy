...
{
    public class QuestConditionsManager
    {
        // -------------------------------
        // UPDATE THESE FOR THIS TYPE
        // -------------------------------
        private QuestConditionsDrawer _drawer = ScriptableObject.CreateInstance<QuestConditionsDrawer>();
        ...
        private string FinalDestination => LocationToSave<QuestCondition>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        private int NewObjectQuestConditionIndex
        {
            get => EditorPrefs.GetInt($"New Object Quest Condition Index {ThisType}", 0);
            set => EditorPrefs.SetInt($"New Object Quest Condition Index {ThisType}", value);
        }
        ...
        private void ResetCache()
        {
            ...
            SetDictionaryDrawer();
            GameModuleObjectTypes<QuestCondition>(true);
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Quest Conditions")
                ManageObjects();
            
            SetAllDirty<QuestCondition>();
        }
        ...
        {
            foreach(var moduleObject in SearchResults<QuestCondition>(SearchTypeIndex, SearchString))
            {
                // UNIQUE FOR QUEST CONDITIONS & REWARDS
                EditorPrefs.SetBool($"Editor Show Quest Condition {moduleObject.name}", moduleObject.showInManager);
        ...
        private void ShowOn(QuestCondition moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
            ...
            if (!moduleObject.showInManager && KeyShift)
                CloseAllInManager<QuestCondition>();
        ...
        {
            var searchResults = SearchBoxWithType<QuestCondition>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        {
            Label("<b><color=#99ffff>Quest Conditions</color></b> & <b><color=#99ffff>Quest Rewards</color></b> have <b><color=#99ffff>Dictionaries</color></b>, as do all <b><color=#99ffff>ModulesScriptableObject</color></b> objects, " +
        ...
        private void ManageCreateNew()
        {
            ...
            if (ButtonWordWrap($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    ...
                    var selectedTypeName = ClassTypes()[NewObjectQuestConditionIndex];
                    ...
                    Debug.Log($"selectedTypeName = {selectedTypeName}");
                    Debug.Log(objectType.BaseType);
                    Debug.Log(typeof(ModulesScriptableObject));
        ...
        private List<string> ClassTypes()
        {
            ...
            var i = 0;
            while (i < EditorPrefs.GetInt("QuestConditionDrawerTypes"))
            {
                typeList.Add(EditorPrefs.GetString($"QuestConditionDrawerType{i}"));
                i++;
            }
        ...
        {
            if (!EditorPrefs.HasKey("QuestConditionDrawerTypes"))
            {
                Label("No Quest Condition Types Found.");
                return;
            }
            ...
            if (NewObjectQuestConditionIndex > typeArray.Length)
                NewObjectQuestConditionIndex = 0;
            
            NewObjectQuestConditionIndex = Popup(NewObjectQuestConditionIndex, displayList.ToArray(), 250);
        }
