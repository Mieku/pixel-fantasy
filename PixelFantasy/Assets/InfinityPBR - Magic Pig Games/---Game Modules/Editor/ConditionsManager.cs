...
namespace InfinityPBR.Modules
{
    public class ConditionsManager
    {
        ...
        // -------------------------------
        private ConditionsDrawer _drawer = ScriptableObject.CreateInstance<ConditionsDrawer>();
        ...
        private string FinalDestination => LocationToSave<Condition>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        // New version -- March 5, 2024 -- not copied to the other module types yet. Just slightly more efficient.
        private string SaveLocation
        {
            get
            {
                var defaultValue = DefaultPathValue();
                var value = EditorPrefs.GetString(_objectManagerSaveLocationKey, defaultValue);
                return string.IsNullOrWhiteSpace(value) ? defaultValue : value; 
            }
            set => EditorPrefs.SetString(_objectManagerSaveLocationKey, value);
        }
        ...
        private void ResetCache()
        {
            ...
            GameModuleObjects<Condition>(true);
            ...
            DebugConsoleMessage("Conditions Cache Reset");
        }
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Conditions")
                ManageObjects();
            
            SetAllDirty<Condition>();
        }
        ...
        private void DrawResultsHeader()
        {
            ...
            Label("", 25);
            Label("", 25);
            Label("", 250, false, false, true);
            Label("<b>Timing</b>", 120, false, false, true);
            Label("<b>Period</b>", 40, false, false, true);
            Label("<b>Expiration Condition</b>", 150, false, false, true);
            EndRow();
        }
        ...
        private void DrawResults()
        {
            DrawResultsHeader();
            GreyLine(1, 0);
            foreach(var moduleObject in SearchResults<Condition>(SearchTypeIndex, SearchString))
            {
        ...
        private void ShowOff(Condition moduleObject)
        {
            StartRow();
            ...
            // More here
            ShowTime(moduleObject);
            ShowExpirationCondition(moduleObject);
            
            EndRow();
        }

        private void ShowOn(Condition moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
        ...
        private readonly string[] _timeOptions = {
            "Timed", "Timed Periodic", "Instant", "Infinite", "Infinite Periodic"
        };
        
        private string TimeOptionSelected
        {
            get => EditorPrefs.GetString($"{ThisType} Time Option Selected", "Timed");
            set => SetString($"{ThisType} Time Option Selected", value);
        }
        
        private int TimeOptionIndex(string selected) => _timeOptions.ToList().IndexOf(selected);
        
        private void ShowTime(Condition moduleObject)
        {
            var timeOption = "";
            
            if (moduleObject.Infinite && !moduleObject.Periodic) timeOption = "Infinite";
            if (moduleObject.Infinite && moduleObject.Periodic) timeOption = "Infinite Periodic";
            if (moduleObject.Instant) timeOption = "Instant";
            if (!moduleObject.Instant && !moduleObject.Infinite && !moduleObject.Periodic) timeOption = "Timed";
            if (!moduleObject.Instant && !moduleObject.Infinite && moduleObject.Periodic) timeOption = "Timed Periodic";

            var tempOption = timeOption;
            
            timeOption = _timeOptions[Popup(TimeOptionIndex(timeOption), _timeOptions, 120)];

            if (timeOption != tempOption)
            {
                if (timeOption == "Infinite")
                {
                    if (KeyShift) GameModuleObjectsOfType(moduleObject.objectType).ToList().ForEach(SetInfinite);
                    else SetInfinite(moduleObject);
                }
                if (timeOption == "Infinite Periodic")
                {
                    if (KeyShift) GameModuleObjectsOfType(moduleObject.objectType).ToList().ForEach(SetInfinitePeroidic);
                    else SetInfinitePeroidic(moduleObject);
                }
                if (timeOption == "Instant")
                {
                    if (KeyShift) GameModuleObjectsOfType(moduleObject.objectType).ToList().ForEach(SetInstant);
                    else SetInstant(moduleObject);
                }
                if (timeOption == "Timed")
                {
                    if (KeyShift) GameModuleObjectsOfType(moduleObject.objectType).ToList().ForEach(SetTimed);
                    else SetTimed(moduleObject);
                }
                if (timeOption == "Timed Periodic")
                {
                    if (KeyShift) GameModuleObjectsOfType(moduleObject.objectType).ToList().ForEach(SetTimedPeriodic);
                    else SetTimedPeriodic(moduleObject);
                }
            }

            if (timeOption == "Infinite Periodic" || timeOption == "Timed Periodic")
            {
                ColorsIf(moduleObject.Period == 0, Color.red, Color.white, Color.red, Color.white);
                moduleObject.SetPeriod(Float(moduleObject.Period, 40));
                ResetColor();
            }
            else
                Label("", 40);
        ...
        private void SetInfinite(Condition moduleObject)
        {
            moduleObject.SetInfinite(true);
            moduleObject.SetInstant(false);
            moduleObject.SetPeriodic(false);
        }
        
        private void SetInfinitePeroidic(Condition moduleObject)
        {
            moduleObject.SetInfinite(true);
            moduleObject.SetInstant(false);
            moduleObject.SetPeriodic(true);
        }
        
        private void SetInstant(Condition moduleObject)
        {
            moduleObject.SetInfinite(false);
            moduleObject.SetInstant(true);
        }
        
        private void SetTimed(Condition moduleObject)
        {
            moduleObject.SetInfinite(false);
            moduleObject.SetInstant(false);
            moduleObject.SetPeriodic(false);
        ...
        private void SetTimedPeriodic(Condition moduleObject)
        {
            moduleObject.SetInfinite(false);
            moduleObject.SetInstant(false);
            moduleObject.SetPeriodic(true);
        ...
        private void ShowExpirationCondition(Condition moduleObject)
        {
            var text = "";
            if (moduleObject.ExpirationCondition != null)
            {
                text = $"{text}{symbolMultiplyCircle}";// <b>{moduleObject.ExpirationCondition.name}</b>";
            }
            
            //Label($"{text}", 20, false, false, true);
            if (moduleObject.ExpirationCondition != null)
                moduleObject.SetExpirationCondition(Object(moduleObject.ExpirationCondition, typeof(Condition), 150) as Condition);
        }
        ...
        {
            var searchResults = SearchBoxWithType<Condition>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        private void ManageCreateNew()
        {
            ...
            if (Button($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    var newObjectType = CreateAssets<Condition>(FinalDestination, NewObjectType, NewObjectTypeIndex, NewObjectsNameArray(NewObjectNames));
                    GameModuleObjectsOfType<Condition>(newObjectType, true);
                    ...
                    SearchTypeIndex = GameModuleObjectTypes<Condition>(true).ToList().IndexOf(newObjectType);
                    GameModuleObjectsOfType<Condition>(newObjectType, true);
                    ExitGUI();
                }
            }
