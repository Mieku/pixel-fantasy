...
{
    public class MasteryLevelsManager
    {
        // -------------------------------
        // UPDATE THESE FOR THIS TYPE
        // -------------------------------
        private MasteryLevelsDrawer _drawer = ScriptableObject.CreateInstance<MasteryLevelsDrawer>();
        ...
        private string FinalDestination => LocationToSave<MasteryLevels>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Mastery Levels")
                ManageObjects();
            
            SetAllDirty<MasteryLevels>();
        }
        ...
        {
            foreach(var moduleObject in SearchResults<MasteryLevels>(SearchTypeIndex, SearchString))
            {
        ...
        private void ShowOff(MasteryLevels moduleObject)
        {
            StartRow();
            ...
            // More here
            ShowLevelCount(moduleObject);
            
            EndRow();
        }

        private void ShowOn(MasteryLevels moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
            ...
            if (!moduleObject.showInManager && KeyShift)
                CloseAllInManager<MasteryLevels>();
        ...
        private void ShowLevelCount(MasteryLevels moduleObject)
        {
            Label($"<color=#999999><b>{moduleObject.levels.Count}</b></color> <color=#777777>Levels</color>", 100, false, false, true);
        }
        
        private void DrawSearchBox()
        {
            var searchResults = SearchBoxWithType<MasteryLevels>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        private void ManageCreateNew()
        {
            ...
            if (Button($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    ...
                    var newObjectType = CreateAssets<MasteryLevels>(FinalDestination, NewObjectType, NewObjectTypeIndex, NewObjectsNameArray(NewObjectNames));
                    ...
                    GameModuleObjectsOfType<MasteryLevels>(newObjectType, true);
                    ...
                    SearchTypeIndex = GameModuleObjectTypes<MasteryLevels>(true).ToList().IndexOf(newObjectType);
                    GameModuleObjectsOfType<MasteryLevels>(newObjectType, true);
