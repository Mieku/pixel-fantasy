...
{
    public class LookupTablesManager
    {
        // -------------------------------
        // UPDATE THESE FOR THIS TYPE
        // -------------------------------
        private LookupTablesDrawer _drawer = ScriptableObject.CreateInstance<LookupTablesDrawer>();
        const string ThisType = "Lookup Table";
        ...
        private string FinalDestination => LocationToSave<LookupTable>(NewObjectTypeIndex, SaveLocation, NewObjectType);
        ...
        private void ResetCache()
        {
            ...
            SetDictionaryDrawer();
            GameModuleObjectTypes<LookupTable>(true);
            GameModuleObjects<LookupTable>(true);
        ...
        public void Draw(Rect area)
        {
            ...
            if (MainMenuSelected == "Lookup Tables")
                ManageObjects();
            
            SetAllDirty<LookupTable>();
        }

        private void CheckOrders()
        {
            foreach (var table in GameModuleObjects<LookupTable>())
            {
                if (table.table.Count(x => x.manualOrder == 0) == 1) continue;
                for (var index = 0; index < table.table.Count; index++)
                {
                    var item = table.table[index];
        ...
        {
            foreach(var moduleObject in SearchResults<LookupTable>(SearchTypeIndex, SearchString))
            {
        ...
        private void ShowOff(LookupTable moduleObject)
        {
            StartRow();
            ...
            // More here
            ShowCount(moduleObject);
            
            EndRow();
        }

        private void ShowOn(LookupTable moduleObject)
        {
            StartVerticalBox();
            StartRow();
            moduleObject.showInManager = OnOffButton(moduleObject.showInManager);
        ...
        private void ShowCount(LookupTable moduleObject)
        {
            Label($"<color=#999999><b>{moduleObject.table.Count}</b></color> <color=#777777>values</color>", 80, false, false, true);
        }
        
        private void DrawSearchBox()
        {
            var searchResults = SearchBoxWithType<LookupTable>(SearchTypeIndex, GameModuleObjectTypes());
        ...
        {
            Label("<b><color=#99ffff>Lookup Table</color></b> objects have <b><color=#99ffff>Dictionaries</color></b>, as do all <b><color=#99ffff>ModulesScriptableObject</color></b> objects, " +
        ...
        private void ManageCreateNew()
        {
            ...
            if (Button($"Create {NamesInArray(NewObjectNames)} {ClassNamePlural} of type {typeName}", 300, 50))
            {
                if (canCreate)
                {
                    ...
                    var newObjectType = CreateAssets<LookupTable>(FinalDestination, NewObjectType, NewObjectTypeIndex, NewObjectsNameArray(NewObjectNames));
                    ...
                    GameModuleObjectsOfType<LookupTable>(newObjectType, true);
                    ...
                    SearchTypeIndex = GameModuleObjectTypes<LookupTable>(true).ToList().IndexOf(newObjectType);
                    GameModuleObjectsOfType<LookupTable>(newObjectType, true);
