...
namespace InfinityPBR.Modules.Loot
{
    ...
    public class LootBoxEditor : Editor
    {
        ...
        private LootBoxesDrawer _modulesDrawer;
        
        private void OnEnable()
        {
            _modulesObject = (LootBox)target;
            _modulesDrawer = CreateInstance<LootBoxesDrawer>();
        ...
        private int copiedUid = 0;
        
        // Used in the management of the inspector script
        private int _selectedLootBoxItemsIndex = 0;
        private int lootItemsIndex = 0;
        private List<LootItems> cachedLootItems = new List<LootItems>();
        private List<string> cachedLootItemsNames = new List<string>();

        private List<ItemObject> testItems = new List<ItemObject>();
        ...
        {
            cachedLootItems.Clear();
            cachedLootItemsNames.Clear();
            
            // Get all LootItems in the project, and cache them here
            foreach (var lootItems in GameModuleObjects<LootItems>(true))
            {
                cachedLootItems.Add(lootItems);
                cachedLootItemsNames.Add(lootItems.objectName);
            }
        }
        
        protected override void Cache()
        {
            //Debug.Log("Loot Box Cache");
            //Setup();
        }
        ...
        private void ShowTest()
        {
            ...
            {
                Label(itemObject.FullName(false));
            }
        ...
        private void DisplayItemsToSpawnHeader()
        {
            ...
            Label($"Item Object {symbolInfo}", toolTip, 145, true);
            ...
            for (var index = 0; index < _itemsSettings.activeAttributeTypes.Count; index++)
            {
                ...
                Label($"{activeAllowedAttributeType} {symbolInfo}", toolTipAttribute, 117, true);
            }
             ...
             */
            /*
            foreach (string typeName in _itemsSettings.attributeTypes)
            {
                var labelName = typeName;
                var toolTip = "Select which items, if any, may spawn. If multiple are selected, the spawn will " +
                              "choose a random object from the selections. There must be at least one Item Object.";
                labelName = $"{typeName} {symbolInfo}";
                
                //Label(labelName, toolTip, typeName == "Item Object" ? 148 : 175, true);
                Label(labelName, toolTip, 145, true);
            }
            */
            /* 
             EndRow();
         ...
         private void DisplayItemsToSpawn(int itemsSettingsIndex)
         {
             ...
             for (int i = 0; i < _itemsSettings.itemsToSpawn.Count; i++)
             {
                 ...
                 /*
                  // OLD CODE BELOW
                 foreach (string columnName in _itemsSettings.attributeTypes)
                 {
                     if (columnName == "Item Object")
                     {
                         ShowItemSelections(i, itemsSettingsIndex);
                         continue;
                     }
                     ShowAttributeSelections(columnName, i, itemsSettingsIndex);
                 }
                 *//*
                ShowDeleteButton(i);
                EndRow();
        ...
        void ToggleSelectAllAttributes(LootBoxItemToSpawn itemToSpawn, string objectType)
        {
            ...
            //itemToSpawn.GetItemAttributes(objectType).Clear();
            //foreach (var itemAttribute in GameModuleObjectsOfType<ItemAttribute>(objectType))
            //    itemToSpawn.AddItemAttribute(objectType, itemAttribute);
        }
        ...
        void ToggleItemAttributeSelected(object itemName)
        {
            ...
            if (objectName == "All")
            {
                ...
                //itemToSpawn.SaveAttributeDictionary();
                return;
            }
            ...
            // Otherwise add it
            availableItemAttributesList.attributes.Add(GameModuleObjectsOfType<ItemAttribute>(objectType).FirstOrDefault(x => x.name == objectName));
            
            
            /*
            var itemAttribute = GameModuleObjectsOfType<ItemAttribute>(objectType).FirstOrDefault(x => x.name == objectName);
            
            if (itemToSpawn.GetItemAttributes(objectType).Contains(itemAttribute))
            {
                itemToSpawn.GetItemAttributes(objectType).RemoveAll(x => x == itemAttribute);
                itemToSpawn.SaveAttributeDictionary();
                return;
            }

            itemToSpawn.GetItemAttributes(objectType).Add(itemAttribute);
            itemToSpawn.SaveAttributeDictionary();
            *//*
        }
        ...
        private void ShowItemSelections(int itemToSpawnIndex, int itemsSettingsIndex)
        {
            ...
            var label = countSelected == 1 ? itemToSpawn.itemObjects[0].objectName : $"Random of {countSelected}";
        ...
        private void ShowAttributeSelections(string attributeType, int itemToSpawnIndex, int itemsSettingsIndex)
        {
            ...
            var label = countSelected == 0 ? "None" : countSelected == 1 ? thisAttributeList.attributes[0].objectName : $"Random of {countSelected}";
        ...
        {
            if (cachedLootItemsNames.Count == 0) return;
            
            BackgroundColor(Color.yellow);
            StartVerticalBox();
            
            Label("Add Loot Items to this Loot Box", true);
            
            StartRow();
            lootItemsIndex = Popup(lootItemsIndex, cachedLootItemsNames.ToArray(), 200);
            if (Button("Add", 200))
            {
                Undo.RecordObject(_modulesObject, "Undo");
                AddLootItems(cachedLootItems[lootItemsIndex]);
                DoCache();
            }
            EndRow();

            LootItems lootItems = cachedLootItems[lootItemsIndex];
