    ...
    public static class VGUI
    {
        ...
        public abstract class CustomPopupWindow : EditorWindow
        {
            ...
            public void HeaderButtonsGUI<T>(Rect headerRect, bool showPinButton = true, System.Action<T> initOnPinAction = null) where T : CustomPopupWindow
            {
                ...
                void pin()
                {
                    if (!isPinned && crossRect.IsHovered()) return;

                    GUI.color = isPinned ? Color.white : pinRect.IsHovered() ? Greyscale(.9f) : Greyscale(.65f);
                    ...
                    isPinned = !isPinned;
                    var w = CustomPopupWindow.Create<T>(!isPinned, position.position);
                    initOnPinAction?.Invoke(w);
                ...
                void cross()
                {
                    ...
                    GUI.color = Greyscale(crossRect.IsHovered() ? .9f : .65f);
            ...
            {
                if (rect.IsHovered() && eType == EventType.MouseDrag)
                {
                    isDragged = true;
            ...
            public static T Create<T>(bool closeOnFocusLost = true, Vector2 position = default) where T : CustomPopupWindow
            {
                ...
                if (closeOnFocusLost)
                    window.ShowAsDropDown(Rect.zero.SetPos(position), new Vector2(window.initWidth, window.initHeight));
                else
                    window.ShowPopup();
