	...
	public class TileWorldCreator : MonoBehaviour
	{
		...
		WorldMap AddNewGeneratedBlueprintMap(string _guid, int _width, int _height)
		{
			...
			foreach (var _cluster in _existingMap.clusters.Keys)
			{
				...
				_tmpMap.clusters.Add(_cluster, new Dictionary<Vector2Int, TileDataStruct>());
				
				foreach (var _position in _existingMap.clusters[_cluster].Keys)
				{
					TileDataStruct _tileData = new TileDataStruct();
					TileDataStruct _existingData = _existingMap.clusters[_cluster][_position];
		...
		public bool[,] GetMapOutputFromBlueprintLayer(System.Guid _guid)
		{
			for (int i = 0; i < twcAsset.mapBlueprintLayers.Count; i ++)
			{
				if (twcAsset.mapBlueprintLayers[i].guid.CompareTo(_guid) == 0)
				{
					return twcAsset.mapBlueprintLayers[i].map;
				}
			}
		...
		/// <returns></returns>
		public TileDataStruct GetTileData(string _layerName, Vector3 _position)
		{
			TileDataStruct _tileData = new TileDataStruct(); //null;
		...
		void ExecuteBlueprintLayer(int _layerIndex)
		{
			...
			{	
				...
				if (_action.active)
				{
					...
					bool _result = false;
					for (int x = 0; x < twcAsset.mapWidth; x ++)
					{
						for (int y = 0; y < twcAsset.mapHeight; y ++)
						{
							if (_map[x,y])
							{
								_result = true;
		...
		void CreateFinalBlueprintMap(TileWorldCreatorAsset.BlueprintLayerData _layerData)
		{
			...
			var _mapSizeX = _layerData.map.GetLength(0);
			var _mapSizeY = _layerData.map.GetLength(1);
			
			// Subdivide the map so that a single tile (1x1) turns in to four tiles (2x2).
			_subdMap = TileWorldCreatorUtilities.SubdivideMap(_layerData.map);
			...
			var _finalMap = AddNewGeneratedBlueprintMap(_layerData.guid.ToString() + "_UNSUBD", _mapSizeX, _mapSizeY);
			
			var _subdMapSizeX = _subdMap.GetLength(0);
			var _subdMapSizeY = _subdMap.GetLength(1);
			
			
			// Add data to our final map
			for (int x = 0; x < _subdMapSizeX; x ++)
			{
				for (int y = 0; y < _subdMapSizeY; y ++)
				{
			...
			for (int x = 0; x < _mapSizeX; x ++)
			{
				for (int y = 0; y < _mapSizeY; y ++)
				{
					...
					if (_layerData.map[x,y])
					{
						AddTileData(_finalMap, _layerData.map, x, y, false);
					}
				}
			}
		}
		...
		void AddTileData(WorldMap _tilemap, bool[,] _map, int _x, int _y, bool _isSubd)
		{
			...
			_tileData.neighboursLocation = TileWorldCreatorUtilities.GetNeighboursLocation(_map, _x, _y, _tileData.neighboursLocation);
			// var _location = TileWorldCreatorUtilities.GetNeighboursLocation(_map, _x, _y);
			var _yRotation = TileWorldCreatorUtilities.ReturnTileRotation(_tileData.neighboursLocation);
			...
			{
				_tileData.tileType = TileDataStruct.TileType.exteriorCorner;
			}
			// CORNER
			...
			{
				// CORNER
				if (_tileData.neighboursLocation .northWest && _tileData.neighboursLocation .north && _tileData.neighboursLocation .northEast && _tileData.neighboursLocation .east && _tileData.neighboursLocation .southEast)
				{			
					_tileData.tileType = TileDataStruct.TileType.exteriorCorner;
				}
				// CORNER
				if (_tileData.neighboursLocation .southWest && _tileData.neighboursLocation .south && _tileData.neighboursLocation .southEast && _tileData.neighboursLocation .east && _tileData.neighboursLocation .northEast)
				{
					_tileData.tileType = TileDataStruct.TileType.exteriorCorner;
				}
				// CORNER
				if (_tileData.neighboursLocation .southWest && _tileData.neighboursLocation .west && _tileData.neighboursLocation .northWest && _tileData.neighboursLocation .north && _tileData.neighboursLocation .northEast)
				{
					_tileData.tileType = TileDataStruct.TileType.exteriorCorner;
				}
				// CORNER
				if (_tileData.neighboursLocation .southWest && _tileData.neighboursLocation .south && _tileData.neighboursLocation .southEast && _tileData.neighboursLocation .west && _tileData.neighboursLocation .northWest)
				{
					_tileData.tileType = TileDataStruct.TileType.exteriorCorner;
				}
								
				// STRAIGHT
				if (_tileData.neighboursLocation .west && _tileData.neighboursLocation .east && _tileData.neighboursLocation .southWest && _tileData.neighboursLocation .south && _tileData.neighboursLocation .southEast)
				{
					_tileData.tileType = TileDataStruct.TileType.edge;
				}
				// STRAIGHT
				if (_tileData.neighboursLocation .west && _tileData.neighboursLocation .east && _tileData.neighboursLocation .northWest && _tileData.neighboursLocation .north && _tileData.neighboursLocation .northEast)
				{
					_tileData.tileType = TileDataStruct.TileType.edge;
				}
				// STRAIGHT
				if (_tileData.neighboursLocation .west && _tileData.neighboursLocation .northWest && _tileData.neighboursLocation .north && _tileData.neighboursLocation .southWest && _tileData.neighboursLocation .south)
				{
					_tileData.tileType = TileDataStruct.TileType.edge;
				}
				// STRAIGHT
				if (_tileData.neighboursLocation .east && _tileData.neighboursLocation .north && _tileData.neighboursLocation .northEast && _tileData.neighboursLocation .south && _tileData.neighboursLocation .southEast)
				{
					_tileData.tileType = TileDataStruct.TileType.edge;
				}
			}
			...
			{
				_tileData.tileType = TileDataStruct.TileType.interiorCorner;
			}
			// FULL
			if (_neigboursCount == 9)
			{
				_tileData.tileType = TileDataStruct.TileType.fill;
			}
			...
			for (int x = 0; x < _x + 2; x ++)
			{
				for (int y = 0; y < _y + 2; y ++)
				{
					...
					if (twcAsset.mapOrientation == TileWorldCreatorAsset.MapOrientation.XZ)
					{
						...
						//  _pos = new Vector3(transform.localPosition.x + (x * (_clusterSize * 0.5f) + (_clusterSize * 0.5f)), transform.localPosition.y + 0.2f, transform.localPosition.z + (y * (_clusterSize * 0.5f) + (_clusterSize * 0.5f)));
						// _pos = new Vector3(transform.localPosition.x + (_clusterSize * 0.25f) + (x * (_clusterSize * 0.5f)) , transform.localPosition.y + 0.2f, transform.localPosition.z + (_clusterSize * 0.25f) + (y * (_clusterSize * 0.5f)));
