...
namespace TWC.Actions
{
	...
	{	
		...
		IEnumerator InstantiateIE(TileWorldCreator _twc, bool _forceRebuild)
		{
			...
			if (tileMapTMP != null)
			{
				for (int c = 0; c < tileMap.clusters.Keys.Count; c ++)
				// foreach (var _cluster in tileMap.clusters.Keys)
				{
					...
					int _key = tileMap.clusters.Keys.ToArray()[c];
					for (int p = 0; p < tileMap.clusters[_key].Keys.Count; p ++)
					// foreach (var _position in tileMap.clusters[tileMap.clusters.Keys[c]].Keys)
					{
						var _position = tileMap.clusters[_key].Keys.ToArray()[p];
						if (tileMapTMP.clusters.ContainsKey(_key))
						{
							if (tileMapTMP.clusters[_key].ContainsKey(_position))
							{
							}
							else
							{	
			...
			// Debug.Log("Cluster count: " + modifiedClusters.Count);
					...
					{				
						...
						foreach (var _position in tileMap.clusters[modifiedClusters[c]].Keys)
						{
							if (!IgnoreTileCheck(_position, _twc))
							{
								...
								TileWorldCreator4TilesPreset _tilePreset = GetRandomTilesPreset(_twc, modifiedClusters[c] + (_position.x + _position.y));
				...
				foreach (var _cluster in tileMap.clusters.Keys)
				{
					...
					foreach (var _position in tileMap.clusters[_cluster].Keys)
					{
						if (!IgnoreTileCheck(_position, _twc))
						{
							...
							TileWorldCreator4TilesPreset _tilePreset = GetRandomTilesPreset(_twc, _cluster + (_position.x + _position.y));
							GameObject _tileObject = null;
		...
		GameObject InstantiateTile(TileDataStruct _tileData, TileWorldCreator4TilesPreset _tilePreset, GameObject _clusterObject, TileWorldCreator _twc)
		{
			...
			{
				case TileDataStruct.TileType.edge:
					...
					break;
				case TileDataStruct.TileType.exteriorCorner:
					...
					break;
				case TileDataStruct.TileType.interiorCorner:
					...
					break;
				case TileDataStruct.TileType.fill:
		...
		public TileWorldCreator4TilesPreset GetRandomTilesPreset(TileWorldCreator _twc, int _tileSeed)
		{
			// Count all weights
			if (tiles.Count == 0)
				return null;
			...
			UnityEngine.Random.InitState(_tileSeed);
